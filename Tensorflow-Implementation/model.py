...
def masked_ce_loss(real, pred):
    ...
    :return: loss: returns the mean loss
...
def create_model(num_tags):
    ...
    """
    ## BERT encoder
    encoder = TFBertModel.from_pretrained(config_tf.BASE_MODEL_PATH)
    # encoder =  config_tf.encoder
    ## NER Model
    input_ids = layers.Input(shape=(config_tf.MAX_LEN,), dtype=tf.int32)
    token_type_ids = layers.Input(shape=(config_tf.MAX_LEN,), dtype=tf.int32)
    attention_mask = layers.Input(shape=(config_tf.MAX_LEN,), dtype=tf.int32)
