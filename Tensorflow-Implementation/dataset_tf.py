...
def create_inputs_targets(data_csv):
    ...
    for sentence, tag in zip(sentences, tags):
        ...
        for idx, word in enumerate(sentence):
            ids = config_tf_.tokenizer.encode(word, add_special_tokens=False)
        ...
        # Pad truncate
        input_ids = input_ids[:config_tf.MAX_LEN - 2]
        target_tags = target_tags[:config_tf.MAX_LEN - 2]

        input_ids = [101] + input_ids + [102]
        target_tags = [16] + target_tags + [16]
        token_type_ids = [0] * len(input_ids)
        attention_mask = [1] * len(input_ids)
        padding_len = config_tf.MAX_LEN - len(input_ids)
        ...
        assert len(target_tags) == config_tf.MAX_LEN, f'{len(input_ids)}, {len(target_tags)}'
...
def create_test_input_from_text(texts):
    ...
    for sentence in texts:
        ...
        for idx, word in enumerate(sentence.split()):
            ids = config_tf.tokenizer.encode(word, add_special_tokens=False)
            input_ids.extend(ids.ids)
            num_tokens = len(ids)
