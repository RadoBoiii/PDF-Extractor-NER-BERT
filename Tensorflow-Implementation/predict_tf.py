"""
Access the file from Tensorflow Implementation folder.
...
def get_tokens(tokenized_sentence,tags,tags_name):
  """This function extracts tokens for the words in the sentences.
    ...
    :return: map: returns encoded tokens of each word
...
def get_mapping(sentence):
  """Maps the words to the tokens and returns the predictions of each tag.
    ...
    :param meta_file: Meta file from the config_tf.py, defaults to none
    
    :type data_path: .bin file
    
    :param model_bert: NER BERT model from config_tf.py, defaults to none
  ...
  x_test, n_tokens = dataset_tf.create_test_input_from_text(sentence)
  ...
  for i in map:
        print(i,"-->",config_tf.tokenizer.decode(map[i]))
        if i == "B-ACCOUNT #":
          extracted_info['Account_no'].append(config_tf.tokenizer.decode(map[i]))
          final_info['Account_no'].append(extracted_info['Account_no'][0])

        if i == "B-ADMIT DATE":
          extracted_info['Admit_Date'].append(config_tf.tokenizer.decode(map[i]))
          final_info['Admit_Date'].append(extracted_info['Admit_Date'][0])

        if i == "B-ADMITTING DIAGNOSIS":
          extracted_info['Admitting_Disease'].append(config_tf.tokenizer.decode(map[i]))
          extracted_info['Admitting_Disease'][0] = extracted_info['Admitting_Disease'][0].split()[0]
        if i == "I-ADMITTING DIAGNOSIS":
          disease = config_tf.tokenizer.decode(map[i])
          copy = config_tf.tokenizer.decode(map[i])
          if len(copy.split()) > 1:
            dis = disease.split()[0]
        ...
        if i == "B-ADMITTING PHYSICIAN":
          extracted_info['Admitting_Physician'].append(config_tf.tokenizer.decode(map[i]))
          extracted_info['Admitting_Physician'][0] = extracted_info['Admitting_Physician'][0].split()[0]
        if i == "I-ADMITTING PHYSICIAN":
          last_name = config_tf.tokenizer.decode(map[i]).split()
        ...
        if i == "B-AGE":
          extracted_info['Age'].append(config_tf.tokenizer.decode(map[i]))
          final_info['Age'].append(extracted_info['Age'][0])

        if i == "B-BIRTH DATE":
          extracted_info['DOB'].append(config_tf.tokenizer.decode(map[i]))
          final_info['DOB'].append(extracted_info['DOB'][0])

        if i == "B-MR NUMBER":
          extracted_info['MR_Number'].append(config_tf.tokenizer.decode(map[i]))
        ...
        if i == "B-PATIENT NAME":
          print(config_tf.tokenizer.decode(map[i]))
          extracted_info['Patient_Name'].append(config_tf.tokenizer.decode(map[i]))
        ...
        if i == "B-PATIENT PHONE #":
          extracted_info['Patient_Phone_no'].append(config_tf.tokenizer.decode(map[i]))
        ...
        if i == "B-PRIMARY INSURANCE PLAN":
          ip = config_tf.tokenizer.decode(map[i])
          print(ip);
          # extracted_info['Primary_Insurance_Policy'].append(config_tf.tokenizer.decode(map[i]))
          if ip == "":
        ...
        if i == "I-PRIMARY INSURANCE PLAN":
          iip = config_tf.tokenizer.decode(map[i])
          print(iip)
          # extracted_info['Primary_Insurance_Policy'].append(config_tf.tokenizer.decode(map[i]))
          if iip == "":
            # print("inside")
        ...
        if i == "B-SOCIAL SECURITY #":
          extracted_info['Social_Security_no'].append(config_tf.tokenizer.decode(map[i]))
...
if __name__ == "__main__":
    ...
    m1.load_weights(config_tf.WEIGHT_PATH)
    TEXT_FILE_PATH = r'/content/drive/MyDrive/EdgeML_Team/bert-entity-extraction/dataset_tf/Text /*.txt'
    ...
    print(extracted_df)
    extracted_df.to_csv(config_tf.EXTRACTED_FILE, header = True, index = False)
